...
public class PaymentMethodAggregate(Guid id) : AggregateRootBase(id)
{
    ...
    private PaymentMethodAggregate(Guid id, PaymentProvider provider, string name, string code, TypePaymentMethod type, string? comments)
        : this(id)
    {
    ...
    public static PaymentMethodAggregate Create(Guid id, PaymentProvider provider, string name, string code, TypePaymentMethod type, string? comments)
    {
        return new PaymentMethodAggregate(id, provider, name, code, type, comments);
    }

    public void Update(PaymentProvider provider, string name, string code, TypePaymentMethod type, string? comments, bool isActive)
    {
        DomainGuard.IsNullOrEmpty(name, Errors.NameOfPaymentMethodCannotBeNullOrEmpty);
        DomainGuard.IsNullOrEmpty(code, Errors.CodeOfPaymentMethodCannotBeNullOrEmpty);
        DomainGuard.IsGreaterThan(code.Length, 32, Errors.CodeOfPaymentMethodCannotBeGreaterThan32Characters);
