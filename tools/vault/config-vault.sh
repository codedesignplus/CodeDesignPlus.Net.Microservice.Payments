...
vault kv put -mount=security-codedesignplus-keyvalue ms-payments \
...
vault kv get -mount=security-codedesignplus-keyvalue ms-payments

# Write database configuration
echo "$newlines"
echo "${BLUE}7. Writing database configuration...${NC}"
vault write security-codedesignplus-database/config/db-ms-payments \
    plugin_name=mongodb-database-plugin \
    allowed_roles="ms-payments-mongo-role" \
    connection_url="mongodb://{{username}}:{{password}}@mongo:27017/admin?ssl=false" \
    username="admin" \
    password="password"

vault write security-codedesignplus-database/roles/ms-payments-mongo-role \
    db_name=db-ms-payments \
    creation_statements='{ "db": "admin", "roles": [{ "role": "readWrite", "db": "db-ms-payments" }] }' \
    default_ttl="1h" \
    max_ttl="24h"

vault read security-codedesignplus-database/creds/ms-payments-mongo-role
...
vault write security-codedesignplus-rabbitmq/roles/ms-payments-rabbitmq-role \
    vhosts='{"/":{"write": ".*", "read": ".*", "configure": ".*"}}'

vault read security-codedesignplus-rabbitmq/creds/ms-payments-rabbitmq-role