...
1. Build the Docker image using the Dockerfile in the REST API entry point:
   ```bash
   docker build -t ms-payments-rest . -f src/entrypoints/CodeDesignPlus.Net.Microservice.Payments.Rest/Dockerfile

   docker run -d -p 5000:5000 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-payments-rest ms-payments-rest
   ```

2. Build the Docker image using the Dockerfile in the gRPC entry point:
   ```bash
   docker build -t ms-payments-grpc . -f src/entrypoints/CodeDesignPlus.Net.Microservice.Payments.gRpc/Dockerfile

   docker run -d -p 5001:5001 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-payments-grpc ms-payments-grpc
   ```

3. Build the Docker image using the Dockerfile in the Worker entry point:
   ```bash
   docker build -t ms-payments-worker . -f src/entrypoints/CodeDesignPlus.Net.Microservice.Payments.AsyncWorker/Dockerfile

   docker run -d -p 5002:5002 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-payments-worker ms-payments-worker
   ```
